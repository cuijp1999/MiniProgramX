...
class TestProductApi:
    def test_product_classify_api(self):
        ...
        assert False not in [i in res.text for i in ["id", "name", "topic_img_id", "img", "url"]]
    ...
    def test_classify_product_api(self):
        ...
        utils.common_assert_code(res)
        assert len(res.json()) > 0
        assert False not in [i in res.text for i in ["id", "name", "price", "stock", "main_img_url", "img_id"]]
